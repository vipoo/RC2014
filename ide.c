...
static void ide_fault(struct ide_drive *d, const char *p) { fprintf(stderr, "ide: %s: %d: %s\n", d->controller->name, (int)(d - d->controller->drive), p); }
...
int ide_attach(struct ide_controller *c, int drive, int fd) {
  ...
  d->fd = fd;
  if (read(d->fd, d->data, 512) != 512 || read(d->fd, d->identify, 512) != 512) {
