...
void ppide_write(struct ppide *ppide, uint8_t addr, uint8_t val) {
  ...
  case 2: /* Port C - address/control lines */
    ...
    if (dlow & 0x20) {
      if (ppide->trace)
        fprintf(stderr, "write edge: %02X = %04X\n", d, ((uint16_t)ppide->pioreg[1] << 8) | ppide->pioreg[0]);
...
void ppide_reset(struct ppide *ppide) { ide_reset_begin(ppide->ide); }
...
void ppide_trace(struct ppide *ppide, int onoff) { ppide->trace = onoff; }

int ppide_attach(struct ppide *ppide, int drive, int fd) { return ide_attach(ppide->ide, drive, fd); }
