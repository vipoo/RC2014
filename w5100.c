...
void nic_w5100_socket_end(nic_w5100_socket_t *socket) { nic_w5100_socket_reset(socket); }

static void w5100_write_socket_mr(nic_w5100_socket_t *socket, uint8_t b) {
...
static void w5100_socket_open(nic_w5100_socket_t *socket_obj) {
  if ((socket_obj->mode == W5100_SOCKET_MODE_UDP || socket_obj->mode == W5100_SOCKET_MODE_TCP) && socket_obj->state == W5100_SOCKET_STATE_CLOSED) {
    ...
    if (socket_obj->fd == -1) {
      fprintf(stderr, "w5100: failed to open %s socket for socket %d; errno %d: %s\n", description, socket_obj->id, errno, strerror(errno));
      return;
    }
    fcntl(socket_obj->fd, F_SETFL, FNDELAY);

    if (setsockopt(socket_obj->fd, SOL_SOCKET, SO_REUSEADDR, &one, sizeof(one)) == -1) {
      fprintf(stderr, "w5100: failed to set SO_REUSEADDR on socket %d; errno %d: %s\n", socket_obj->id, errno, strerror(errno));
    }
...
static int w5100_socket_bind_port(nic_w5100_t *self, nic_w5100_socket_t *socket) {
...
static void w5100_socket_listen(nic_w5100_t *self, nic_w5100_socket_t *socket) {
...
static void w5100_socket_connect(nic_w5100_t *self, nic_w5100_socket_t *socket) {
  if (socket->state == W5100_SOCKET_STATE_INIT) {
    struct sockaddr_in sa;
    ...
    if (connect(socket->fd, (struct sockaddr *)&sa, sizeof(sa)) == -1) {
      if (errno != EINPROGRESS) {
        fprintf(stderr, "w5100: failed to connect socket %d to 0x%08x:0x%04x; errno %d: %s\n", socket->id, ntohl(sa.sin_addr.s_addr), ntohs(sa.sin_port), errno, strerror(errno));
...
static void w5100_socket_close(nic_w5100_t *self, nic_w5100_socket_t *socket) {
  if (socket->fd != -1) {
    close(socket->fd);
    socket->fd           = -1;
    socket->socket_bound = 0;
...
static void w5100_socket_discon(nic_w5100_t *self, nic_w5100_socket_t *socket) {
  if (socket->state == W5100_SOCKET_STATE_ESTABLISHED || socket->state == W5100_SOCKET_STATE_CLOSE_WAIT) {
    socket->ir |= 1 << 1;
    socket->state = W5100_SOCKET_STATE_CLOSED;
...
static void w5100_socket_send(nic_w5100_t *self, nic_w5100_socket_t *socket) {
  if (socket->state == W5100_SOCKET_STATE_UDP) {
    ...
    socket->datagram_lengths[socket->datagram_count++] = socket->tx_wr - socket->last_send;
...
static void w5100_socket_recv(nic_w5100_t *self, nic_w5100_socket_t *socket) {
  if (socket->state == W5100_SOCKET_STATE_UDP || socket->state == W5100_SOCKET_STATE_ESTABLISHED || socket->state == W5100_SOCKET_STATE_CLOSE_WAIT) {
...
static void w5100_write_socket_cr(nic_w5100_t *self, nic_w5100_socket_t *socket, uint8_t b) {
...
static void w5100_write_socket_port(nic_w5100_t *self, nic_w5100_socket_t *socket, int which, uint8_t b) {
...
uint8_t nic_w5100_socket_read(nic_w5100_t *self, uint16_t reg) {
  ...
  case W5100_SOCKET_SR:
    ...
    break;
  case W5100_SOCKET_PORT0:
  case W5100_SOCKET_PORT1:
    ...
    break;
  case W5100_SOCKET_TX_FSR0:
  case W5100_SOCKET_TX_FSR1:
    ...
    break;
  case W5100_SOCKET_TX_RR0:
  case W5100_SOCKET_TX_RR1:
  ...
  case W5100_SOCKET_TX_WR1:
    ...
    break;
  case W5100_SOCKET_RX_RSR0:
  case W5100_SOCKET_RX_RSR1:
...
void nic_w5100_socket_write(nic_w5100_t *self, uint16_t reg, uint8_t b) {
  nic_w5100_socket_t *socket     = &self->socket[(reg >> 8) - 4];
  int                 socket_reg = reg & 0xff;
  ...
  case W5100_SOCKET_PORT1:
    ...
    break;
  case W5100_SOCKET_DIPR0:
  case W5100_SOCKET_DIPR1:
  case W5100_SOCKET_DIPR2:
  case W5100_SOCKET_DIPR3:
    ...
    break;
  case W5100_SOCKET_DPORT0:
  case W5100_SOCKET_DPORT1:
...
uint8_t nic_w5100_socket_read_rx_buffer(nic_w5100_t *self, uint16_t reg) {
...
void nic_w5100_socket_write_tx_buffer(nic_w5100_t *self, uint16_t reg, uint8_t b) {
...
void nic_w5100_socket_add_to_sets(nic_w5100_socket_t *socket, fd_set *readfds, fd_set *writefds, int *max_fd) {
  if (socket->fd != -1) {
    /* We can process a UDP read if we're in a UDP state and there are at least
       ...
       data). */
    int udp_read = socket->state == W5100_SOCKET_STATE_UDP && 0x800 - socket->rx_rsr >= 9;
    /* We can process a TCP read if we're in the established state and have
       any room in our buffer (no header necessary for TCP). */
    int tcp_read = socket->state == W5100_SOCKET_STATE_ESTABLISHED && 0x800 - socket->rx_rsr >= 1;
...
static void w5100_socket_process_accept(nic_w5100_socket_t *socket) {
  struct sockaddr_in sa;
...
static void w5100_socket_process_read(nic_w5100_socket_t *socket, nic_w5100_t *self) {
...
static void w5100_socket_process_udp_write(nic_w5100_socket_t *socket) {
  ssize_t            bytes_sent;
  ...
  if (bytes_sent == length) {
    if (--socket->datagram_count)
      memmove(socket->datagram_lengths, &socket->datagram_lengths[1], 0x1f * sizeof(int));
...
static void w5100_socket_process_tcp_write(nic_w5100_socket_t *socket) {
  ssize_t  bytes_sent;
  int      offset = socket->tx_rr & 0x7ff;
...
void nic_w5100_socket_process_io(nic_w5100_socket_t *socket, fd_set readfds, fd_set writefds, nic_w5100_t *self) {
...
void w5100_process(nic_w5100_t *self) {
  ...
  for (i = 0; i < 4; i++) {
    nic_w5100_socket_add_to_sets(&self->socket[i], &readfds, &writefds, &max_fd);
...
static void w5100_write_mr(nic_w5100_t *self, uint8_t b) {
  ...
  if (b & 0x3C)
    fprintf(stderr, "w5100: unsupported value 0x%02x written to MR\n", b);
}

static void w5100_write_imr(nic_w5100_t *self, uint8_t b) {
  nic_w5100_debug("w5100: writing 0x%02x to IMR\n", b);
  /* FIXME */
  if (b != 0xef)
    fprintf(stderr, "w5100: unsupported value 0x%02x written to IMR\n", b);
}

static void w5100_write__msr(nic_w5100_t *self, uint16_t reg, uint8_t b) {
  const char *regname = reg == W5100_RMSR ? "RMSR" : "TMSR";

  nic_w5100_debug("w5100: writing 0x%02x to %s\n", b, regname);

  if (b != 0x55)
    fprintf(stderr, "w5100: unsupported value 0x%02x written to %s\n", b, regname);
}
