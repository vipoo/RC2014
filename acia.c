...
void acia_set_input(struct acia *acia, int onoff) { acia->input = onoff; }
...
uint8_t acia_irq_pending(struct acia *acia) { return acia->inint; }
...
void acia_free(struct acia *acia) { free(acia); }

void acia_trace(struct acia *acia, int onoff) { acia->trace = onoff; }
