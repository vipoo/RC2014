...
#define e86_get_reg8(cpu, reg) ((((reg)&4) ? ((cpu)->dreg[(reg)&3] >> 8) : (cpu)->dreg[(reg)&3]) & 0xff)
...
#define e86_set_ax(cpu, val)                \
  do {                                      \
    (cpu)->dreg[E86_REG_AX] = (val)&0xffff; \
  } while (0)
#define e86_set_bx(cpu, val)                \
  do {                                      \
    (cpu)->dreg[E86_REG_BX] = (val)&0xffff; \
  } while (0)
#define e86_set_cx(cpu, val)                \
  do {                                      \
    (cpu)->dreg[E86_REG_CX] = (val)&0xffff; \
  } while (0)
#define e86_set_dx(cpu, val)                \
  do {                                      \
    (cpu)->dreg[E86_REG_DX] = (val)&0xffff; \
  } while (0)
#define e86_set_sp(cpu, val)                \
  do {                                      \
    (cpu)->dreg[E86_REG_SP] = (val)&0xffff; \
  } while (0)
#define e86_set_bp(cpu, val)                \
  do {                                      \
    (cpu)->dreg[E86_REG_BP] = (val)&0xffff; \
  } while (0)
#define e86_set_si(cpu, val)                \
  do {                                      \
    (cpu)->dreg[E86_REG_SI] = (val)&0xffff; \
  } while (0)
#define e86_set_di(cpu, val)                \
  do {                                      \
    (cpu)->dreg[E86_REG_DI] = (val)&0xffff; \
  } while (0)
...
#define e86_set_cs(cpu, val)                \
  do {                                      \
    (cpu)->sreg[E86_REG_CS] = (val)&0xffff; \
  } while (0)
#define e86_set_ds(cpu, val)                \
  do {                                      \
    (cpu)->sreg[E86_REG_DS] = (val)&0xffff; \
  } while (0)
#define e86_set_es(cpu, val)                \
  do {                                      \
    (cpu)->sreg[E86_REG_ES] = (val)&0xffff; \
  } while (0)
#define e86_set_ss(cpu, val)                \
  do {                                      \
    (cpu)->sreg[E86_REG_SS] = (val)&0xffff; \
  } while (0)

#define e86_get_ip(cpu)     ((cpu)->ip)
#define e86_get_cur_ip(cpu) ((cpu)->cur_ip)
#define e86_set_ip(cpu, val)  \
  do {                        \
    (cpu)->ip = (val)&0xffff; \
  } while (0)
...
#define e86_get_linear(seg, ofs) ((((seg)&0xffffUL) << 4) + ((ofs)&0xffff))

static inline unsigned char e86_get_mem8(e8086_t *c, unsigned short seg, unsigned short ofs) {
  unsigned long addr = e86_get_linear(seg, ofs) & c->addr_mask;
  return ((addr < c->ram_cnt) ? c->ram[addr] : c->mem_get_uint8(c->mem, addr));
}

static inline void e86_set_mem8(e8086_t *c, unsigned short seg, unsigned short ofs, unsigned char val) {
  unsigned long addr = e86_get_linear(seg, ofs) & c->addr_mask;
...
static inline unsigned short e86_get_mem16(e8086_t *c, unsigned short seg, unsigned short ofs) {
  unsigned long addr = e86_get_linear(seg, ofs) & c->addr_mask;
...
static inline void e86_set_mem16(e8086_t *c, unsigned short seg, unsigned short ofs, unsigned short val) {
  unsigned long addr = e86_get_linear(seg, ofs) & c->addr_mask;
...
#define e86_get_prt8(cpu, ofs) (cpu)->prt_get_uint8((cpu)->prt, ofs)

#define e86_get_prt16(cpu, ofs) (cpu)->prt_get_uint16((cpu)->prt, ofs)
...
void e86_set_mem(e8086_t *c, void *mem, e86_get_uint8_f get8, e86_set_uint8_f set8, e86_get_uint16_f get16, e86_set_uint16_f set16);

void e86_set_prt(e8086_t *c, void *prt, e86_get_uint8_f get8, e86_set_uint8_f set8, e86_get_uint16_f get16, e86_set_uint16_f set16);
