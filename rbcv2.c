...
static void prop_write(uint8_t addr, uint8_t val) {
  ...
  case 2:
    ...
    case 0x30: /* READ */
      ...
      prop_err = 0;
      prop_st  = 0;
      if (lseek(prop_fd, lba, SEEK_SET) < 0 || read(prop_fd, prop_sbuf, 512) != 512) {
        prop_err = -6;
    ...
    case 0x50: /* WRITE */
      ...
      prop_err = 0;
      prop_st  = 0;
      if (lseek(prop_fd, lba, SEEK_SET) < 0 || write(prop_fd, prop_sbuf, 512) != 512) {
        prop_err = -6;
...
static void ramf_init(void) {
  ...
  ramf_addr = mmap(NULL, 8192 * 1024, PROT_READ | PROT_WRITE, MAP_SHARED, ramf_fd, 0L);
