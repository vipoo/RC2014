...
static void m6800_cpu_state(struct m6800 *cpu) { fprintf(stderr, "%04X : %6s %02X|%02X %04X %04X | ", cpu->pc, m6800_flags(cpu), cpu->a, cpu->b, cpu->x, cpu->s); }
...
static char *opmap[256] = {
    ...
    "INCB", "TSTB", "T(HCF)", "CLRB",

    /* 0x60 */
    "NEG x", NULL, NULL, "COM x,X",

    "LSR x,X", NULL, "ROR x,X", "ASR x,X",

    "ASL x,X", "ROL x,X", "DEC x,X", NULL,

    "INC x,X", "TST x,X", "JMP x,X0", "CLR x,X",

    /* 0x70 */
    "NEG e", NULL, NULL, "COM e",

    "LSR e", NULL, "ROR e", "ASR e",

    "ASL e", "ROL e", "DEC e", NULL,

    "INC e", "TST e", "JMP e0", "CLR e",

    /* 0x80 */
    "SUBA #i", "CMPA #i", "SBCA #i", "SUBD #ii",

    "ANDA #i", "BITA #i", "LDAA #i", NULL,

    "EORA #i", "ADCA #i", "ORAA #i", "ADDA #i",

    "CPX #ii", "BSR b", "LDS #ii", NULL,

    /* 0x90 */
    "SUBA d", "CMPA d", "SBCA d", "SUBD d2",

    "ANDA d", "BITA d", "LDAA d", "STAA d",

    "EORA d", "ADCA d", "ORAA d", "ADDA d",

    "CPX d2", "JSR d0", "LDS d2", "STS d2",

    /* 0xA0 */
    "SUBA x,X", "CMPA x,X", "SBCA x,X", "SUBD x,X",

    "ANDA x,X", "BITA x,X", "LDAA x,X", "STAA x,X",

    "EORA x,X", "ADCA x,X", "ORAA x,X", "ADDA x,X",

    "CPX x,X2", "JSR x,X0", "LDS x,X2", "STS x,X2",

    /* 0xB0 */
    "SUBA e", "CMPA e", "SBCA e", "SUBD e",

    "ANDA e", "BITA e", "LDAA e", "STAA e",

    "EORA e", "ADCA e", "ORAA e", "ADDA e",

    "CPX e2", "JSR e0", "LDS e2", "STS e2",

    /* 0xC0 */
    "SUBB #i", "CMPB #i", "SBCB #i", "ADDD #ii",

    "ANDB #i", "BITB #i", "LDAB #i", NULL,

    "EORB #i", "ADCB #i", "ORAB #i", "ADDB #i",

    "LDD #ii", NULL, "LDX #ii", NULL,

    /* 0xD0 */
    "SUBB d", "CMPB d", "SBCB d", "ADDD d2",

    "ANDB d", "BITB d", "LDAB d", "STAB d",

    "EORB d", "ADCB d", "ORAB d", "ADDB d",

    "LDD d2", "STD d2", "LDX d2", "STX d2",

    /* 0xE0 */
    "SUBB x,X", "CMPB x,X", "SBCB x,X", "ADDD x,X",

    "ANDB x,X", "BITB x,X", "LDAB x,X", "STAB x,X",

    "EORB x,X", "ADCB x,X", "ORAB x,X", "ADDB x,X",

    "LDD x,X2", "STD x,X2", "LDX x,X2", "STX x,X2",

    /* 0xF0 */
    "SUBB e", "CMPB e", "SBCB e", "ADDD e",

    "ANDB e", "BITB e", "LDAB e", "STAB e",

    "EORB e", "ADCB e", "ORAB e", "ADDB e",

    "LDD e2", "STD e2", "LDX e2", "STX e2"};
...
static void m6800_push(struct m6800 *cpu, uint8_t val) { m6800_do_write(cpu, cpu->s--, val); }
...
static uint8_t m6800_pull(struct m6800 *cpu) { return m6800_do_read(cpu, ++cpu->s); }
...
static int m6800_execute_one(struct m6800 *cpu) {
  ...
  if (cpu->type == CPU_68HC11) {
    while ((opcode & 0xFF) == 0x18 || (opcode & 0xFF) == 0x1A || (opcode & 0xFF) == 0xCD) {
      switch (opcode) {
      case 0x18:
        table = 1;
        break;
  ...
  case 0x14: /* BSET direct */
    data8 = m6800_do_read(cpu, cpu->pc++);
    m6800_do_write(cpu, data8, m6800_do_read(cpu, data8) | m6800_do_read(cpu, cpu->pc++));
    return clocks;
...
void m6800_clear_interrupt(struct m6800 *cpu, int irq) { cpu->irq &= ~irq; }

void m6800_raise_interrupt(struct m6800 *cpu, int irq) { cpu->irq |= irq; }
...
uint8_t m6hc11_read_io(struct m6800 *cpu, uint8_t addr) {
  ...
  case 0x13:
    return cpu->io.tic2 & 0xFF : case 0x14 : return cpu->io.tic3 >> 8;
  case 0x15:
...
void m68hc11_write_io(struct m6800 *cpu, uint8_t addr, uint8_t val) {
  ...
  case 0x16: /* Timer output compare */
    cpu->io.toc1 &= 0xFF : cpu->io.toc1 |= val;
    break;
  case 0x17:
    cpu->io.toc1 &= 0xFF00;
    cpu->io.toc1 |= val;
    break;
  case 0x18:
    cpu->io.toc2 &= 0xFF : cpu->io.toc2 |= val << 8;
    break;
  case 0x19:
    cpu->io.toc2 &= 0xFF00;
    cpu->io.toc2 |= val;
    break;
  case 0x1A:
    cpu->io.toc3 &= 0xFF : cpu->io.toc3 |= val << 8;
    break;
  case 0x1B:
    cpu->io.toc3 &= 0xFF00;
    cpu->io.toc3 |= val;
    break;
  case 0x1C:
    cpu->io.toc4 &= 0xFF : cpu->io.toc4 |= val << 8;
    break;
  case 0x1D:
    cpu->io.toc4 &= 0xFF00;
    cpu->io.toc4 |= val;
    break;
  case 0x1E:
    cpu->io.toc5 &= 0xFF : cpu->io.toc5 |= val << 8;
    break;
  case 0x1F:
    cpu->io.toc5 &= 0xFF00;
    cpu->io.toc5 |= val;
