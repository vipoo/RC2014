...
static uint8_t z80_dma_one_cycle(struct z80dma *dma) {
  ...
  if (dma->reg[RR1] == dma->reg[WR_LEN_L] && dma->reg[RR2] == dma->reg[WR_LEN_H]) {
    /* Completed */
    dma->enabled  = 0;
    dma->forcerdy = 0;
    dma->reg[RR0] |= 1;
...
void z80dma_free(struct z80dma *dma) { free(dma); }

void z80dma_trace(struct z80dma *dma, int on) { dma->trace = on; }
