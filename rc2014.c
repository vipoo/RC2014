...
void recalc_interrupts(void) { int_recalc = 1; }
...
static const uint8_t sd_csd[17] = {
    ...
    0x00, 0x2D, 0x00, 0x32, 0x13, 0x59, 0x83, 0xB1, 0xF6, 0xD9, 0xCF, 0x80, 0x16, 0x40, 0x00, 0x00};
...
static uint8_t sd_process_command(void) {
  ...
  case 0x40 + 17: /* Read */
    ...
    sd_out[0] = 0xFF;
    sd_out[1] = 0xFE;
    sd_lba    = sd_cmd[4] + 256 * sd_cmd[3] + 65536 * sd_cmd[2] + 16777216 * sd_cmd[1];
    if (trace & TRACE_SD)
...
static uint8_t sd_process_data(void) {
  ...
  case 0x40 + 24: /* Write */
    sd_mode = 0;
    if (lseek(sd_fd, sd_lba, SEEK_SET) < 0 || write(sd_fd, sd_in, 512) != 512) {
      if (trace & TRACE_SD)
...
static uint8_t pio_remap[4] = {0, 2, 1, 3};

/* With the C/D and A/B lines the other way around */
static void pio_write2(uint8_t addr, uint8_t val) { pio_write(pio_remap[addr], val); }

static uint8_t pio_read2(uint8_t addr) { return pio_read(pio_remap[addr]); }
